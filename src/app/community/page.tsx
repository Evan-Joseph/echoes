...
function PublicCheckInCard({
  ...
  const handleToggleLike = async () => {
    ...
    try {
      const result = await action(checkIn.id, currentUser.id);
      if (!result.success) throw new Error('Failed to update like status.');
    } catch (error) {
      toast({
        title: '操作失败',
        description: '网络开小差了，请稍后再试。',
        variant: 'destructive',
      });
      // Revert on failure
      setIsLiked(wasLiked);
  ...
  const handleReport = async () => {
    ...
    try {
      ...
      toast({
        title: '举报失败',
        description: e.message || '操作失败，请稍后重试。',
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <div className="w-full max-w-2xl space-y-4">
      {[...Array(3)].map((_, i) => (
        <Card
          key={i}
          className="w-full max-w-2xl rounded-2xl border-none bg-card/80 shadow-lg backdrop-blur-sm"
        >
          <CardHeader>
            <div className="flex items-center gap-3">
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="flex flex-col items-center space-y-4 p-4">
        ...
        {!isLoading &&
          !error &&
          posts.map(({ checkIn, author, authorCheckIns }) => (
            <PublicCheckInCard
              key={checkIn.id}
              checkIn={checkIn}
              author={author}
              authorCheckIns={authorCheckIns}
