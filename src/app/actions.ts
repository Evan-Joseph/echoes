...
export async function getPublicCheckInsAction(): Promise<
  { checkIn: AppCheckIn; author: AppUser; authorCheckIns: AppCheckIn[] }[]
> {
...
export async function likeCheckInAction(
  checkInId: string,
  userId: string
...
export async function unlikeCheckInAction(
  checkInId: string,
  userId: string
): Promise<{ success: boolean }> {
  try {
...
export async function makeCheckInPublicAction(
  checkInId: string
): Promise<{ success: boolean }> {
  try {
...
export async function makeCheckInPrivateAction(
  checkInId: string
): Promise<{ success: boolean }> {
  try {
...
export async function generateWordCloudDataAction(input: {
...
export async function generateAiCommentAction(input: {
...
export async function addCommentAction(
  ...
  userId: string,
  content: string
): Promise<{ success: boolean; newComment?: CommentWithAuthor }> {
  try {
...
export async function getPendingReportsAction(): Promise<
  { report: Report; checkIn: AppCheckIn; reporter: AppUser; author: AppUser }[]
> {
  ...
  const populatedReports = await Promise.all(
    reports.map(async (report) => {
      const [checkIn, reporter, author] = await Promise.all([
        ...
        report.checkInId
          ? getCheckInById(report.checkInId).then((c) =>
              c ? getUserProfile(c.userId) : null
            )
          : null,
      ]);
  ...
  return populatedReports.filter(Boolean) as {
    report: Report;
    checkIn: AppCheckIn;
    reporter: AppUser;
    author: AppUser;
...
export async function getUserMessagesAction(
  userId: string
): Promise<Message[]> {
...
export async function leaveActivityAction(
  activityId: string,
  userId: string
): Promise<{ success: boolean }> {
  try {
...
export async function getPendingActivitiesAction(): Promise<
  ActivityWithAuthor[]
> {
...
export async function getActivityByIdAction(
  activityId: string
): Promise<(Activity & { author: AppUser }) | null> {
...
export async function getCheckInsForActivityAction(
  activityId: string
): Promise<{ checkIn: AppCheckIn; author: AppUser }[]> {
...
export async function saveMonthlyReportAction(
