...
function EditCheckInForm({
  ...
  const [photoDataUri, setPhotoDataUri] = React.useState<string | undefined>(
    checkIn.photoUrl
  );
  const [fileName, setFileName] = React.useState<string | null>(
    checkIn.photoUrl ? '保留现有图片' : null
  );
  ...
  return (
    <div className="space-y-4">
      ...
      />
      {photoDataUri && (
        <div className="group relative h-32 w-32 overflow-hidden rounded-md border">
          <Image
            src={photoDataUri}
            alt="当前图片"
            fill
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          >
            <Label
              htmlFor={`photo-upload-${checkIn.id}`}
              className="cursor-pointer text-muted-foreground hover:text-primary"
            >
              <Paperclip className="h-5 w-5" />
              <Input
                id={`photo-upload-${checkIn.id}`}
                type="file"
                accept="image/*"
                className="sr-only"
...
function calculateStreak(checkIns: AppCheckIn[]): number {
  ...
  const uniqueDates = Array.from(
    new Set(
      sortedCheckIns.map((c) => startOfDay(new Date(c.createdAt)).getTime())
    )
  );
...
function ProfileCheckInCard({
  ...
  return (
    <Card className="w-full rounded-2xl border-none bg-card/80 shadow-lg backdrop-blur-sm">
      <CardHeader className="flex flex-row items-start justify-between">
        <div>
          <CardTitle className="text-sm font-normal text-muted-foreground">
            打卡于 {timeAgo}
            {checkIn.updatedAt && !isEditing && (
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem
                  ...
                  className={cn(
                    ...
                    isConfirmingDelete &&
                      'bg-destructive/90 text-destructive-foreground focus:bg-destructive focus:text-destructive-foreground'
                  )}
                >
        ...
        {checkIn.activityId && checkIn.activityTitle && (
          <div className="mb-3">
            <Link href={`/activity/${checkIn.activityId}`}>
              <Badge
                variant="outline"
                className="cursor-pointer border-accent/50 bg-accent/30 text-xs font-normal hover:bg-accent/50"
              >
...
function calculateWeeklyStats(checkIns: AppCheckIn[]) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const checkInsOnDate = checkIns.filter(
      (c) => startOfDay(new Date(c.createdAt)).getTime() === date.getTime()
    );
    stats.push({
...
function AchievementCard({
  ...
  return (
    <Card
      className={cn(
        'w-full border-dashed bg-card/60 backdrop-blur-sm transition-all',
        isUnlocked ? 'border-accent bg-accent/30' : 'border-border/50'
      )}
    >
...
export default function ProfilePage() {
  ...
  const loadAllData = React.useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : '无法加载你的记录，请稍后再试。';
  ...
  const handleUpdateCheckIn = async (
    ...
    try {
      const result = await updateCheckInAction(checkInId, user.id, data);
      if (result.success) {
  ...
  const handleDeleteCheckIn = async (checkInId: string) => {
    ...
    try {
      const result = await deleteCheckInAction(checkInId, user.id);
      if (!result.success) {
  ...
  const handleVisibilityChange = async (
    ...
    const newCheckIns = originalCheckIns.map((c) =>
      c.id === checkInId ? { ...c, isPublic: newVisibility } : c
    );
    setCheckIns(newCheckIns);
    setUserTitle(getUserTitle(userProfile!, newCheckIns));

    const action = newVisibility
      ? makeCheckInPublicAction
      : makeCheckInPrivateAction;
      ...
      <main className="flex flex-col items-center space-y-4 p-4">
        {isLoading || isAuthLoading ? (
          ...
          <>
            ...
            <Card className="w-full max-w-2xl rounded-2xl border-none bg-card/80 shadow-lg backdrop-blur-sm">
              <CardHeader className="items-center text-center">
                <div className="group relative">
                  <Avatar className="h-24 w-24 border-2 border-transparent transition-all group-hover:border-primary">
                    <AvatarImage
                      src={userProfile.photoURL}
                      alt={userProfile.displayName}
                    />
                    <AvatarFallback>
                      {userProfile.displayName.charAt(0) || '旅'}
                    </AvatarFallback>
                  </Avatar>
                  <Button
                    variant="outline"
            ...
            <h2 className="w-full max-w-2xl pt-6 text-lg font-semibold">
              我的成就
            </h2>
