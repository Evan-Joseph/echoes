...
async function uploadPhoto(
  ...
  try {
    ...
    throw new Error(
      `Could not upload photo. Reason: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export async function addCheckIn(checkInData: AppCheckInData): Promise<string> {
  try {
    ...
    if (checkInData.photoDataUri) {
      docData.photoUrl = await uploadPhoto(
        checkInData.photoDataUri,
        checkInData.userId,
        'checkIns'
      );
    }
    ...
    throw new Error(
      `Could not add check-in. Reason: ${e instanceof Error ? e.message : String(e)}`
    );
  }
}
...
export async function updateCheckIn(
  ...
  try {
    ...
    if (data.photoDataUri && data.photoDataUri.startsWith('data:image')) {
      updateData.photoUrl = await uploadPhoto(
        data.photoDataUri,
        userId,
        'checkIns'
      );
      if (existingCheckIn.photoUrl) {
        const oldPhotoPath = new URL(existingCheckIn.photoUrl).pathname.split(
          '/checkIns/'
        )[1];
    ...
    throw new Error(
      `Could not update check-in. Reason: ${e instanceof Error ? e.message : String(e)}`
    );
  }
}
...
export async function deleteCheckIn(
  ...
  if (checkInData.photoUrl) {
    try {
      const photoPath = new URL(checkInData.photoUrl).pathname.split(
        '/checkIns/'
      )[1];
 ...
 */
async function setCheckInPublic(
  checkInId: string,
  isPublic: boolean
): Promise<void> {
  const { error } = await supabase
  ...
  if (error)
    throw new Error(
      `Could not update check-in privacy. Reason: ${error.message}`
    );
}
...
export async function getUserProfile(userId: string): Promise<AppUser> {
  ...
  return { ...data, id: data.id } as AppUser;
}
...
export async function addComment(
  ...
  const { data, error } = await supabase
    .from('comments')
    .insert({ checkInId, userId, content })
    .select('id')
    .single();
  if (error) throw error;
...
export async function getCommentsForCheckIn(
  ...
  return comments.map((c) => ({
    ...c,
    id: String(c.id),
    author: { ...c.author, id: c.author.id },
...
async function updateActivityParticipants(
  activityId: string,
  userId: string,
  action: 'join' | 'leave'
): Promise<void> {
...
export const joinActivity = (activityId: string, userId: string) =>
  updateActivityParticipants(activityId, userId, 'join');
export const leaveActivity = (activityId: string, userId: string) =>
  updateActivityParticipants(activityId, userId, 'leave');
...
export async function getAllUsers(): Promise<AppUser[]> {
  const { data, error } = await supabase
    ...
    .order('createdAt', { ascending: false });
  if (error) throw error;
  return data.map((d) => ({ ...d, id: d.id })) as AppUser[];
}
