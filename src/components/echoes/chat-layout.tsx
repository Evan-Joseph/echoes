...
const WelcomeBanner = ({ onHide }: { onHide: () => void }) => (
  <div className="relative mb-4 space-y-2 rounded-2xl border border-secondary/50 bg-secondary/30 p-4 text-center">
    <Image
...
export default function ChatLayout() {
  ...
  const addMessage = (
    ...
    options?: { data?: any; skipSave?: boolean; contentForDb?: string }
  ): string => {
    if (!user) return '';
    ...
    if (!options?.skipSave) {
      ...
      saveMessage(messageToSave)
        .then((savedId) => {
          setMessages((prev) =>
            prev.map((m) => (m.id === newMessageId ? { ...m, id: savedId } : m))
          );
        })
      ...
      try {
        ...
        const processedMessages = existingMessages.map((msg) => {
          if (msg.data?.newCheckInId) {
            return {
              ...msg,
              content: createSharePrompt(msg.data.newCheckInId),
            };
          }
          return msg;
        });
  ...
  const handleCheckInRetry = async (failedMessage: Message) => {
    ...
    setMessages((prev) =>
      prev.map((m) =>
        m.id === failedMessage.id
          ? { ...m, status: 'pending', content: '正在重新提交...' }
          : m
      )
    );
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((m) => (m.id === failedMessage.id ? successMsg : m))
      );
    } catch (error) {
      ...
      const content = (
        <div>
          ...
          <Button
            ...
            onClick={() => handleCheckInRetry(failedMessage)}
          >
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === failedMessage.id ? { ...m, status: 'failed', content } : m
        )
      );
    } finally {
  ...
  const handleMakePublic = async (checkInId: string, messageId: string) => {
    ...
    setMessages((prev) =>
      prev.map((m) =>
        m.id === messageId
          ? { ...m, data: { ...m.data, actionTaken: true } }
          : m
      )
    );
    ...
    try {
      ...
      if (result.success) {
        addMessage('ai', '太棒了，你的分享可能会鼓励到更多人哦！', {
          data: {},
        });
      } else {
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === messageId
            ? { ...m, data: { ...m.data, actionTaken: false } }
            : m
        )
      );
    } finally {
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2 pt-2">
          <Button
            ...
            disabled={
              messages.find((m) => m.id === messageId)?.data?.actionTaken
            }
          >
  ...
  const processToolResponse = async (tool: {
    name: string;
  }): Promise<{
    content: React.ReactNode;
    contentForDb: string;
    ...
    try {
      ...
      const suggestionsContent = (
        <div className="w-full">
          <p className="mb-3">
            没关系，我们总有灵感枯竭的时候。这里有一些话题，或许能点燃你的思绪：
          </p>
          <Carousel
            ...
            <CarouselContent>
              {suggestions.map((suggestion: string, index: number) => (
                <CarouselItem key={index} className="basis-1/2 sm:basis-1/3">
                  <Button
                    variant="outline"
                    className="h-full w-full justify-start text-wrap text-left"
      ...
      addMessage('ai', suggestionsContent, {
        contentForDb: '为你提供了一些建议。',
        data: {},
      });
    } catch (error) {
      console.error('Failed to get AI suggestions:', error);
      addMessage('ai', '抱歉，我现在脑子有点乱，稍后再试试帮你想主意吧！', {
        data: { isError: true },
      });
    } finally {
      setIsAiResponding(false);
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      let startIndex = 0;
      while (
        startIndex < historyForApi.length &&
        historyForApi[startIndex].role !== 'user'
      ) {
        startIndex++;
      }
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          history: historyForApi.map((m) => ({
            ...
            content:
              m.contentForDb ||
              (typeof m.content === 'string' ? m.content : '[UI Component]'),
          })),
        }),
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Server returned: ${response.status}: ${errorBody.error?.message || 'Unknown error'}`
        );
      }
      ...
      if (aiResult.error) {
        throw new Error(
          aiResult.error.message || 'An unknown error occurred in the API.'
        );
      }

      if (aiResult.responseType === 'tool_use' && aiResult.tool) {
        const { content, contentForDb, skipMessage } =
          await processToolResponse(aiResult.tool);
        if (!skipMessage) {
          addMessage('ai', content, {
            data: { toolName: aiResult.tool.name },
            contentForDb,
          });
        }
        ...
        addMessage('ai', '抱歉，我好像走神了，你能再说一遍吗？', {
          data: { isError: true },
        });
      }
    } catch (error: any) {
  ...
  return (
    <div className="relative flex h-full w-full max-w-2xl flex-col border-border/20 bg-card shadow-2xl shadow-primary/10 sm:my-4 sm:h-[95vh] sm:rounded-2xl">
      <header className="flex items-center justify-between border-b p-4">
        <div className="w-8"></div>
        <div className="flex h-8 flex-grow items-center justify-center">
          <Image
            src="/images/logo/logo-no-bg.svg"
            alt="Echoes Logo"
            width={32}
      ...
      <ScrollArea className="flex-grow p-4" ref={scrollAreaRef}>
        <div className="space-y-6">
          {showWelcome && !isChatLoading && (
            <WelcomeBanner onHide={() => setShowWelcome(false)} />
          )}
          {messages.map((msg) => (
