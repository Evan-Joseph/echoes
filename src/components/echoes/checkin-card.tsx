...
interface CheckInFormProps {
  onCheckIn: (data: Omit<CheckInFormValues, 'activityId'> & { activityId?: string; activityTitle?: string; }) => Promise<void>;
  activities?: Activity[];
...
export function CheckInForm({ onCheckIn, activities = [], preselectedActivityId, isSubmitting, onCancel, editingCheckIn }: CheckInFormProps) {
  ...
  const [fileName, setFileName] = React.useState<string | null>(editingCheckIn?.photoUrl ? '保留现有图片' : null);
  ...
  const onSubmit = async (data: CheckInFormValues) => {
    ...
    if (!editingCheckIn) { // Only reset fully if it's not an edit form
      form.reset();
      setFileName(null);
    }
  };
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="space-y-4 p-1">
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                        <FormControl>
                            <Textarea placeholder="分享你的想法、感悟或成就..." {...field} className="min-h-[100px] bg-background" />
                        </FormControl>
                        <FormMessage />
                        </FormItem>
                    )}
                ...
                {editingCheckIn?.photoUrl && currentPhoto === editingCheckIn.photoUrl && (
                    <div className="relative h-24 w-24 rounded-md overflow-hidden border">
                        <Image src={editingCheckIn.photoUrl} alt="当前图片" fill className="object-cover" data-ai-hint="user content" />
                    </div>
                )}
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <Select onValueChange={field.onChange} defaultValue={field.value} disabled={!!preselectedActivityId || !!editingCheckIn}>
                                    <FormControl>
                                    <SelectTrigger>
            ...
            <div className="flex justify-between items-center mt-4">
                <div className="flex items-center gap-2">
                    <Button type="button" variant="ghost" size="icon" asChild disabled={isSubmitting}>
                    <Label htmlFor="photo-upload-dialog" className="cursor-pointer text-muted-foreground hover:text-primary">
                        <Paperclip className="h-5 w-5" />
                        <Input id="photo-upload-dialog" type="file" accept="image/*" className="sr-only" onChange={handleFileChange} />
                    </Label>
                    </Button>
                    {fileName && <span className="text-xs text-muted-foreground truncate max-w-[100px]">{fileName}</span>}
                </div>
                <div className="flex gap-2">
                    {onCancel && <Button type="button" variant="ghost" onClick={onCancel} disabled={isSubmitting}>取消</Button>}
                    <Button type="submit" className="bg-primary/90 hover:bg-primary text-primary-foreground" disabled={isSubmitting}>
                        {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : editingCheckIn ? '保存修改' : '发布分享'}
                    </Button>
                </div>
            </div>
        </form>
